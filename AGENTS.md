...
| Lint + format check | `pnpm lint` (`biome ci .`) |
...
- `next.config.ts` uses `output: "export"` — the build produces static HTML in `./out`, not a Node.js server. `pnpm start` serves this via `pnpm dlx serve`.
