import { EducationItem } from '../Interfaces/types';

export const education: EducationItem[] = [
  {
    date: 'September 2024 - Ongoing',
    title: 'Bachelors of EngTech & Applied Sciences',
    institution: 'Memorial University of Newfoundland',
    description: [
      'Transferred from Concordia University to Memorial University of Newfoundland to pursue a Bachelors of Engineering Technology and Applied Sciences.',
      'Full-time student while working full-time as a Software QA Specialist & Software Developer @ Fundica.'
    ],
    statsKey: 'Economics',
    link: 'https://www.mun.ca/',
    hoverColors: {
      institution: '#8B0000',
    },
  },
  {
    date: 'August 2021 - May 2024',
    title: 'Computer Science DEC',
    institution: 'John Abbott College',
    description: [
      'Completed a comprehensive 3-year Computer Science DEC program with a focus on software development, mobile app development, and IoT technologies.',
      'Built a strong foundation in programming fundamentals, algorithms, data structures, and computer architecture while developing practical skills in full-stack development.'
    ],
    statsKey: 'Academics',
    link: 'https://www.johnabbott.qc.ca/',
    hoverColors: {
      institution: '#1E3A8A',
    },
  },
];
