import { SnakeSegment } from '../types/types';
...
export const findPath = (
  ...
  while (openList.length > 0 && iterations < maxIterations) {
    ...
    for (const dir of directions) {
      ...
      if (
        neighborX < 0 || neighborX >= viewportWidth ||
        neighborY < 0 || neighborY >= viewportHeight
      ) continue;
      ...
      if (!existingNode || g < existingNode.g) {
        const neighbor: Node = { x: neighborX, y: neighborY, g, h, f, parent: current };
  ...
  if (Math.abs(dx) > Math.abs(dy)) {
    return [{ x: start.x + (dx > 0 ? gridSize : -gridSize), y: start.y, color: '' }];
  }
  return [{ x: start.x, y: start.y + (dy > 0 ? gridSize : -gridSize), color: '' }];
};
