...
const SnakeGame: React.FC<SnakeGameProps> = ({ isActive, onSnakeUpdate }) => {
  ...
  const spawnFood = useCallback((count: number, snake: SnakeSegment[]) => {
    ...
    while (newFood.length < count && attempts < MAX_SPAWN_ATTEMPTS) {
      attempts++;
      const offsetX = (Math.floor(Math.random() * (FOOD_SPAWN_RADIUS * 2 + 1)) - FOOD_SPAWN_RADIUS) * GRID_SIZE;
      const offsetY = (Math.floor(Math.random() * (FOOD_SPAWN_RADIUS * 2 + 1)) - FOOD_SPAWN_RADIUS) * GRID_SIZE;
  ...
  const findNearestFood = useCallback((head: SnakeSegment): FoodTile | null => {
    ...
    return food.reduce((nearest, current) => {
      const nearestDist = manhattanDistance(head.x, head.y, nearest.x, nearest.y);
      const currentDist = manhattanDistance(head.x, head.y, current.x, current.y);
      return currentDist < nearestDist ? current : nearest;
    });
  }, []);

  const checkFoodCollision = useCallback((x: number, y: number): boolean => {
    const foodKey = makeKey(x, y);
    const foodIndex = foodRef.current.findIndex(f => makeKey(f.x, f.y) === foodKey);
  ...
  const findSafeMove = useCallback((head: SnakeSegment, snakeSet: Set<string>, targetFood: FoodTile | null) => {
    const { width, height } = getViewportDimensions();
    
    const safeMoves = DIRECTIONS
      .map(dir => ({ x: head.x + dir.x, y: head.y + dir.y }))
      .filter(pos => 
        pos.x >= 0 && pos.x < width &&
      ...
      .map(pos => ({
        ...pos,
        score: targetFood ? -manhattanDistance(pos.x, pos.y, targetFood.x, targetFood.y) : 0
      }));

    return safeMoves.length > 0 ? safeMoves.sort((a, b) => b.score - a.score)[0] : null;
  }, []);
  ...
  const gameLoop = useCallback(() => {
    ...
    if (pathRef.current.length === 0 && targetFood) {
      ...
      pathRef.current = findPath(head, targetFood, GRID_SIZE, width, height, obstacles);
    }
  ...
  }, [findNearestFood, findSafeMove, checkFoodCollision, handleBoundaryBounce, onSnakeUpdate]);
